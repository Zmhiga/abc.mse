#' Cauchy ABC
#'
#' @description   Approximate Bayesian computation using Cauchy distribution (heavy-tailed) as the data generating
...
#' @param true.sc       The true value of the scale parameter used to generate "observed" data
...
#'
#' @examples cauchy_abc(true.loc = 1, true.sc = 2, prior.loc = c(-5,5), prior.sc = c(0.5,3.5), size = 10^3, nsim = 100, niter = 10^4, epsilon = 0.01)
...
cauchy_abc = function(true.loc = 0, true.sc = 1.5, prior.loc = c(-5,5), prior.sc = c(0.5,3), size = 10^3, nsim = 10^3, niter = 10^4, epsilon = 0.01) {
  ...
  for (i in 1:nsim) {
    ...
    x.obs = rcauchy(size, location = true.loc, scale = true.sc) # generating "observed" data
    ...
    for (j in 1:niter) {  # the larger the number of iterations, the higher the accuracy
      ...
      x.sim = rcauchy(length(x.obs), location = loc.prop, scale = sc.prop) # generating our simulated data
